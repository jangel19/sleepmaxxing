...
output_dir = f"output/week_of_{run_date}"
os.makedirs(output_dir,exist_ok=True)

df = pd.read_csv(csv_path)
df["date"] = pd.to_datetime(df["date"])
# finals_start = pd.to_datetime("2025-12-08")
# finals_end   = pd.to_datetime("2025-12-19") not solely gonna be for finals

highlight_end = df["date"].max()
highlight_start = highlight_end - pd.Timedelta(days = 7)
...
plt.axvspan(highlight_start, highlight_end, color="red", alpha = 0.15)
...
plt.tight_layout()
plt.savefig(f"{output_dir}/hrv_vs_sleep.png", dpi = 150)
plt.close()
...
plt.tight_layout()
plt.savefig(f"{output_dir}/hrv_vs_rhr.png", dpi=150)
plt.close()
...
plt.title("recovery signals over time (HRV, Sleep, RHR)")
...
plt.tight_layout()
plt.savefig(f"{output_dir}/all_metrics_overview.png", dpi=150)
plt.close()

#ml error chart
error_path = "ml_logger/errors.csv"

if os.path.exists(error_path):
    error = pd.read_csv(error_path)
    error["date"] = pd.to_datetime(error["date"], errors="coerce")
    error = error.dropna(subset=["date"])

    plt.figure(figsize=(8,4))
    plt.plot(error["date"], error["error"], "o--")
    plt.axhline(0, color = "gray", linestyle = "--", alpha = 0.6)
    plt.title("ML Prediction Error Over Time")
    plt.ylabel("Prediction Error")
    plt.xlabel("Date")
    plt.tight_layout()
    plt.savefig(f"{output_dir}/ml_error_over_time.png")
    plt.close()
